...
namespace MyGameStore.DAL.Extensions
{
    public static class Extensions
    {
        ...
        private static async Task<List<Person>> LoadPeople()
        {
            ...
            for (int i = 2; i < ws.Dimension.End.Row; i++)
            {
                people.Add(new Person {Id=i-1, LastName=ws.Cells[i,1].Value.ToString(),FirstName=ws.Cells[i,2].Value.ToString(),Email=ws.Cells[i,3].Value?.ToString(), StoreId=null});
            }
